...
| `DelegateBinding`                                                               | Ruled radio rows: serif name, muted meta, selection in `brand`; the no-delegate default row now reads `არ მყავს დელეგატი`.                                                                                                                                           |
...
| `NewsCard`                                                                      | Front-page brief: kicker, serif headline, muted dateline, hairline separation; cover (when present) as a bordered figure with caption rule. Variants: `row` (brief, default) · `tile` (grid card, cover top 3:2) · `lead` (full-width opener, cover 2:1, serif 3xl). |
...
| `LeaderboardDirectory`   | `{ delegates: RankedDelegate[], regions: Region[] }`                     | `/leaderboard`'s filter controls (owner fix #3): name search + region `Select` over the full ranked list, rendering matches through `LeaderRow`. Retired `DelegateDirectory`'s two-column `DelegateCard` split along with the page it lived on — the no-results card and count line carried over byte-identical. Ranks come from `rankDelegates()` over the full list, so filtering never renumbers anyone.                                                        |
| `ReferralCard`           | `{ code, count, teamNote? }`                                             | Callout with the personal referral URL, copy button, QR and the sign-up count. `teamNote` (default true) renders the closing "counts in your team" sentence, and tracks whether the rendered link actually binds a team rather than which page is rendering. `/delegate` keeps the default; `/me` passes `false`; `/me/profile` passes it per role, because an approved delegate reaching that page directly is shown their delegate code, which does bind a team. |
| `StickyBar`              | `{ children }`                                                           | The one mobile bottom bar. `sticky bottom-0` (never `fixed`, which would occlude page ends), 2px ink top rule, paper fill, `env(safe-area-inset-bottom)` padding, `md:hidden`. Both the join CTA and the tab bar render through it, so the bar's look and safe-area handling live in one place. It does **not** enforce one-bar-per-route: that invariant lives at the call sites, which sit in mutually exclusive route groups.                                   |
| `MobileBackHeader`       | `{ href, label }`                                                        | Detail-screen header below `md`: `← უკან` link to a **declared parent**, brand-red small-caps context label at the 0.74rem floor, 2px ink rule. Never `router.back()` — an article opened from a shared link has no history behind it.                                                                                                                                                                                                                             |
| `MobileMenu`             | `{ navItems, sessionSlot?, cta? }`                                       | Full-screen public navigation overlay plus its masthead trigger. Renders only when `navItems` is non-empty, so cabinet and admin keep a bare nameplate. Escape closes, focus is trapped, body scroll locks, focus returns to the trigger.                                                                                                                                                                                                                          |
| `MobileJoinCta`          | —                                                                        | Session-aware sticky CTA. The guest state is the **cached default** and the swap happens client-side after mount, because `app/sw.ts` caches the public shell — a server-rendered session state would reach the next visitor. Self-suppresses on `/join`, `/join/terms`, `/login`.                                                                                                                                                                                 |
| `MobileTabBar`           | `{ tabs, more }`                                                         | Cabinet bottom bar: four destinations plus `მეტი`, 56px, **text only — no icons**, active = brand text over a 2px brand top rule, `Badge` for counts. Labels arrive already shortened from `mobileTabs()`; the component never truncates. Self-suppresses on the membership wizard.                                                                                                                                                                                |
| `MobileMoreSheet`        | `{ items, activeHref?, onClose }`                                        | Overflow bottom sheet; marks the owned destination with `aria-current="page"`, and always carries `← საჯარო` and `გასვლა` on top of any role overflow, which is why every role gets a `მეტი` tab. A real `aria-modal` dialog: focus trapped and restored to its opener.                                                                                                                                                                                            |
| `useFocusTrap`           | `{ active, containerRef, onEscape }`                                     | Shared modal-overlay keyboard behaviour for both overlays: focus into the container, Tab/Shift+Tab cycling, recovery when focus has escaped, Escape, body-scroll lock. Focus _restoration_ is left to the caller, which knows its own trigger.                                                                                                                                                                                                                     |
...
### Mobile tab labels are singular (2026-08-03)

The cabinet tab bar is the one surface where a section's name is singular — ღონისძიება, გამოკითხვა, სიახლე, and პანელი for the delegate panel — while every page heading keeps its plural. That is forced by the text-size floor above, not a style choice: at 0.74rem a twelve-character Georgian plural does not fit a fifth of a 360px screen, and solving the overflow with a smaller font would breach the floor. Overrides live in `TAB_LABELS` (`lib/mobile-nav.ts`); the tab-bar component never truncates. See ADR-027.
