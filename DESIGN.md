...
| `MobileMoreSheet`        | `{ items, onClose }`                                                     | Overflow bottom sheet; always carries `← საჯარო` and `გასვლა` on top of any role overflow, which is why every role gets a `მეტი` tab. A real `aria-modal` dialog: focus trapped and restored to its opener.                                                                                                                                                                                                                                                        |
