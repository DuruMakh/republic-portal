...
| `ReferralCard`           | `{ code, count, teamNote? }`                                             | Callout with the personal referral URL, copy button, QR and the sign-up count. `teamNote` (default true) renders the closing "counts in your team" sentence — delegates keep it (their link binds a team); `/me` and `/me/profile` pass `false` (a member's/registered person's link only counts, binds no team).                                                                                           |
