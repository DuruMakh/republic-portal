...
A branded static `public/og-default.png` (1200×630, brand red, wordmark „ქართული
რესპუბლიკა") used by news-without-cover, events, transparency, and available as the
site-wide fallback. Committed asset, generated once during implementation.
...
- `news`: id uuid PK default, title text (1–160), body text (non-empty), visibility
  ...
  slug text unique (null until first publish; regex + ≤80 check), image_path text
...
- `public_news` — published + `visibility='public'` columns only (id, slug, title,
  body, image_path, published_at). Granted anon + authenticated.
...
- `admin_set_news_image(p_id, p_path)` — pairs with the upload action
  (`news.set_image`).
