...
- [ ] **Step 1:** Change the person-kind generator: `kind: "draft"` → `kind: "registered"`; seeded registered rows get `status: "registered"` WITH a `personal_id` (1-prefixed per seed convention) and NO birth_date/region/city/employment/tier/reference_code (they completed only the light form). Update the `.neq("status", "draft")` assertion to `.neq("status", "registered")` (it excludes non-members from the members assertion — same intent, new name). While in the file, verify seeded `"completed"`-kind rows set `registration_completed_at` — under the new semantics that stamp IS member standing (they have since ADR-016's approval guard; if any path skips it, set it).
